 ...
 * 返回json序列化结果
 */
if (!function_exists('json_suc')) {
    function json_suc($data = [], $message = 'success'): JsonResponse
    {
        $data = [
            'code'  => 0,
            'message' => $message,
 ...
 * 返回json序列化结果
 */
function json_fail($message = 'fail', $data = [], $code = 400): JsonResponse
{
    $data = [
        'code'  => $code,
        'message' => $message,
...
if (!function_exists('apiUser')) {
    function apiUser($seller = 1)
    {
        if ($seller) {
            $user = request()->header('seller_user');
        } else {
            $user = request()->header('admin_user');
        }
...
if (!function_exists('curl')) {
    function curl($url, $data = [], $method = 'GET', $header = [], $decode = true, $json = false)
    {
        if ($method == 'GET') {
        ...
        $message = 'Begin Request:'.$reqNo.';URL:'. $url . ';Data:' . json_encode($data) . ';Method:'. $method;
        wlog($message);
        ...
        $message = 'End Request:'. $reqNo . ';Result:' . $res;
        wlog($message);
